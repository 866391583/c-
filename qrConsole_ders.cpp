...
//        cout<<"abcC"<<endl;
        for(inner=0; inner<=l[idx]; inner++)
            if(sigma[idx][inner]!=-1&&S[mu[idx]-inner]!=-1)
            {
...
void testDecodeRS()
{
    ...
    for(idx=0; idx<Slen; idx++)
    {
        ...
        for(inner=0; inner<rlen; inner++)
            if(r[inner]!=-1)
                ctmp^=alpha_to4[((idx+1)*inner+r[inner])%15];
        S[idx]=index_of4[ctmp];
    }
    cout<<"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"<<endl;
    for(idx=0; idx<10; idx++)
...
//            }
            ...
            int rhoMax=mu_l[rhoIdx];
            for(inner=rhoIdx-1; inner>=0; inner--)
                if(d[inner]!=-1&&mu_l[inner]>rhoMax)
                {
                    rhoIdx=inner;
                    rhoMax=mu_l[rhoIdx];
                }
            ...
            cout<<endl;
            l[idx]=(l[idx-1]>l[rho]+mu[idx-1]-mu[rho])?l[idx-1]:l[rho]+mu[idx-1]-mu[rho];
            cout<<"lidx ###"<<l[idx]<<"  "<<sigma[1][1]<<"  "<<l[rho]<<"#"<<mu[idx-1]<<"#"<<mu[rho]<<endl;
            for(inner=0; inner<=l[idx]; inner++)
                if(sigma[idx-1][inner]==-1||delta[inner]==-1)
                {
                    if(sigma[idx-1][inner]==-1&&delta[inner]!=-1)
...
//        cout<<"abcD"<<endl;
    ...
    {
        ctmp=0;
        for(inner=0; inner<=l[Slen+1]; inner++)
            if(sigma[Slen+1][inner]!=-1)
            {
                mdtmp=idx*inner+sigma[Slen+1][inner];
                while(mdtmp<0)
...
//        nu[idx]=(e[idx]==r[idx])?0:1;
    ...
    for(idx=0; idx<3; idx++)
    {
        ...
        for(inner=idx; inner>=0; inner--)
        {
            if(sigma[Slen+1][idx-inner]!=-1&&S[inner]!=-1)
                ctmp^=alpha_to4[(sigma[Slen+1][idx-inner]+S[inner])%15];
        }
        zeta0[idx]=index_of4[ctmp];
    }
    ...
    for(idx=0; idx<3; idx++)
        if(epos[idx]!=-1)
        {
            ...
            for(inner=0; inner<3; inner++)
            {
                if(zeta0[inner]!=-1)
                {
                    mdtmp=zeta0[inner]-epos[idx]*inner;
                    while(mdtmp<0)
                        mdtmp+=15;
                    ctmp^=alpha_to4[mdtmp%15];
                }
            }
            numerator=index_of4[ctmp];
            denominator=epos[idx];
            for(chi=0; chi<3; chi++)
            {
                if(epos[chi]!=-1&&chi!=idx)
                {
                    mdtmp=epos[chi]-epos[idx];
                    while(mdtmp<0)
                        mdtmp+=15;
                    denominator+=index_of4[0x01^alpha_to4[mdtmp%15]];
                }
            }
            mdtmp=numerator-denominator;
            cout<<endl;
            cout<<"numerator*************"<<numerator<<"denominator***"<<denominator%15<<endl;
            while(mdtmp<0)
                mdtmp+=15;
            e[epos[idx]]=mdtmp%15;
    ...
    for(idx=0; idx<rlen; idx++)
        if(e[idx]!=-1)
        {
            nu[idx]=index_of4[alpha_to4[r[idx]]^alpha_to4[e[idx]]];
        }
        else
            nu[idx]=r[idx];
    cout<<endl;
